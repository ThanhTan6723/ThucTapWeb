...
public class ProductDAO {
    ...
    */
    ...
    public static List<Product> pagingProduct(int cid, int index, String sort, String type) {
        ...
        String query = "";
        int ind = (index - 1) * 8;
        String orderByClause = " ORDER BY " + sort + " " + type + " " + " LIMIT 8 OFFSET" + " " + ind;
        ; // Sắp xếp theo cột được chỉ định theo kiểu được yêu cầu
    ...
    public static List<Product> getListProducts() {
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new Product(rs.getInt(1), rs.getString(2), rs.getDouble(3), rs.getString(4), rs.getString(5),
                        new Category(rs.getInt(6)), rs.getInt(7), rs.getDouble(8), rs.getDouble(9), AccountDAO.getAccountById(rs.getInt(10)),
                        new Provider(rs.getInt(1)), rs.getString(12), rs.getString(13))
                );
            }
        } catch (Exception e) {
